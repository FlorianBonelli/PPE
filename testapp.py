...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("📦 **TabExplorer** by Florian")
...
# ----------------- LAYOUT -----------------
col_form, col_data = st.columns([1.3, 1])
...
with col_form:
    ...
    st.markdown(
    "<hr style='margin-top: 2.3rem; margin-bottom: 3rem; border-color: #e5e7eb;'/>",
    unsafe_allow_html=True,
)
...
with col_data:
    st.markdown(
        '<div class="tabx-section-header"><span class="tabx-section-title">📊 Données générées</span></div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="tabx-helper">Aucune donnée pour le moment.</div>',
        unsafe_allow_html=True,
    )
    #st.info("Les résultats des traitements et des modèles apparaîtront ici une fois le pipeline connecté.")
